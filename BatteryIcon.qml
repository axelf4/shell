...
Image {
	...
	source: {
		...
		const isLow = device.percentage <= 0.20, min = 0, max = 17;
		const w = min + (max - min) * device.percentage;
		const color = isCharging ? C.palette.green80 : isLow ? C.error : C._onSurface;
		`data:image/svg+xml;utf8,<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
